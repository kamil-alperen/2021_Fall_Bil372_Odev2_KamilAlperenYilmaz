...
const filter_müdahaleadi = document.getElementById("filter_müdahaleadi")
...
const item_müdahaleadi = document.getElementsByClassName("item_müdahaleadi")
const prev_item_alanID = []
const prev_item_sinifID = []
const prev_item_müdahaleID = []
const prev_item_müdahaleadi = []
...
const item_error3 = document.getElementsByClassName("label_item_mudahaleID")

let filter_list = []
filter_list[0] = filter_alanID
filter_list[1] = filter_sinifID
filter_list[2] = filter_müdahaleID
filter_list[3] = filter_müdahaleadi

let item_list = []
item_list[0] = item_alanID
item_list[1] = item_sinifID
item_list[2] = item_müdahaleID
item_list[3] = item_müdahaleadi
...
filter_create.addEventListener("click", (e) => {
    if(mandatory_filter()){
        let send_data = {
            ...
            'MüdahaleAdı' : filter_müdahaleadi.value
        }
        fetch('./intervention', {
            method : 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if(data !== "OK"){
                ...
                filter_müdahaleID.value = ''
                filter_müdahaleadi.value = ''
                list_from_zero(1)
            }
        })
    }
...
function add_users_to_table(user_dict){
    for(let i = 0;i < user_dict.length-1;i++){
        ...
        item_müdahaleID[i].value = user_dict[i].MüdahaleID
        item_müdahaleID[i].setAttribute('style','pointer-events:none')
        item_müdahaleadi[i].value = user_dict[i].MüdahaleAdı
        item_müdahaleadi[i].setAttribute('style','pointer-events:none')
        prev_item_alanID[i] = item_alanID[i].value
        prev_item_sinifID[i] = item_sinifID[i].value
        prev_item_müdahaleID[i] = item_müdahaleID[i].value
        prev_item_müdahaleadi[i] = item_müdahaleadi[i].value
    }
}
...
for(let i = 0;i < update_btn.length;i++){
    update_btn[i].addEventListener("click", (e) => {
        if(update_btn[i].value == 'Güncelle'){
            ...
            item_müdahaleID[i].setAttribute('style','pointer-events:all')
            item_müdahaleadi[i].setAttribute('style','pointer-events:all')
        }
        else {
            if(mandatory_item(i)){
                send_data = {
                    ...
                    'MüdahaleAdı' : item_müdahaleadi[i].value
                }
                fetch('./intervention', {
                    method : 'PUT',
                    headers: {
                ...
                .then(data => {
                    ...
                    if(data !== 'OK'){
                        ...
                        prev_item_sinifID[i] = item_sinifID[i].value
                        prev_item_müdahaleID[i] = item_müdahaleID[i].value
                        prev_item_müdahaleadi[i] = item_müdahaleID[i].value
                    }
                })
            }
        }
    ...
    quit_btn[i].addEventListener("click", (e) => {
        if(quit_btn[i].style.opacity == '1'){
            ...
            item_müdahaleID[i].value = prev_item_müdahaleID[i]
            item_müdahaleadi[i].value = prev_item_müdahaleadi[i]
            item_alanID[i].setAttribute('style','pointer-events:none')
            item_sinifID[i].setAttribute('style','pointer-events:none')
            item_müdahaleID[i].setAttribute('style','pointer-events:none')
            item_müdahaleadi[i].setAttribute('style','pointer-events:none')
